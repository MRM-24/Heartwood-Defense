...
function NectarPanel({ s, compact }: { s: GameState; compact?: boolean }) {
...
function WaveTrack({ s, compact }: { s: GameState; compact?: boolean }) {
  return (
    <div
      className={`flex items-center gap-2 rounded-2xl border border-[#3a5a3f] bg-[#101d13]/90 ${
        compact ? 'min-w-0 flex-1 px-2.5 py-1.5' : 'px-3 py-1.5'
      }`}
      role="group"
        ...
        {Array.from({ length: s.waveTotal }).map((_, i) => {
          ...
          return (
            <div
              ...
              className={`flex h-[22px] flex-1 items-center justify-center rounded-md border text-[10px] ${
                done
...
function TrayButton({
  ...
  return (
    ...
    >
      ...
      {/* sprite */}
      <div
        className={`absolute inset-x-2 bottom-6 top-0.5 transition-transform ${usable ? '' : 'opacity-45 grayscale-[.7]'} ${
      ...
      <div className="absolute inset-x-0 bottom-0 flex h-[26px] items-center justify-between bg-[#0c1710]/95 px-1.5">
        ...
        <span className={`font-ui text-[13px] font-extrabold tabular-nums ${afford ? 'text-[#ffd76a]' : 'text-[#ff7d95]'}`}>
          {def.cost}
        </span>
      </div>
      {/* cooldown sweep */}
      {cd > 0 && (
        <div className="absolute inset-0 z-20 flex items-center justify-center" aria-hidden>
          ...
          <span className="relative font-ui text-[20px] font-extrabold text-[#e8f4e4] tabular-nums [text-shadow:0_2px_4px_#000]">
            {cd.toFixed(0)}
          </span>
        </div>
      )}
...
function ShovelButton({ s, onShovel, disabled }: { s: GameState; onShovel: () => void; disabled?: boolean }) {
  return (
    ...
    >
      <Shovel className={`h-7 w-7 ${s.shovelArmed ? 'text-[#ffb37a]' : 'text-[#a8794a]'}`} />
...
export default function Hud({
      ...
      return (
        <div className="flex w-full flex-col gap-2">
          <div className="flex items-stretch gap-2">
            <NectarPanel s={s} compact />
            <WaveTrack s={s} compact />
            <ControlCluster
              speed={speed}
              muted={muted}
              compact
    ...
    return (
      <div className="flex w-full flex-col gap-1.5">
        <div className="flex items-end justify-between gap-2 px-0.5">
          <span className="font-ui text-[11px] font-bold tracking-[0.2em] text-[#78917f]">
            {s.selected
          ...
          <span className="font-ui text-[11px] font-bold tracking-widest text-[#4d6a55] tabular-nums">
            {s.loadout.filter((k) => (s.trayCd[k] ?? 0) <= 0 && s.nectar >= FLORA[k].cost).length}/{s.loadout.length} READY
          </span>
        </div>
  ...
  // ── desktop: the original single-row HUD + tray + controls ──
  return (
    <div className="relative flex w-full items-stretch gap-3">
      ...
      {boss && (
        <div className="absolute left-1/2 top-[118px] z-[95] w-[520px] -translate-x-1/2">
          <BossBar s={s} />
        </div>
      )}
    </div>
